...
class HomePage:
    ...
    def main_menu_bar_wrapper_is_visible(self):
        ...
        for i in range(len(self.main_menu_item_name.all_inner_texts())-2):
            ...
            time.sleep(0.1)
            if self.main_menu_wrapper_column_1.is_visible() or self.main_menu_wrapper_column_2.is_visible() or self.main_menu_wrapper_column_3.is_visible():
                pass
            else:
                 error += 1
        return error == 0
    ...
    def recommended_product_item_price(self) -> str:
        ...
        product_price = float(''.join([i.replace('\xa0zł', '').replace(',', '.').strip(' ') for i in self.recommended_product_price.first.text_content().split('\n')][0:3]))

        Logger.info("Main page price: {}".format(product_price))

        return float(''.join([i.replace('\xa0zł', '').replace(',','.').strip(' ') for i in self.recommended_product_price.first.text_content().split('\n')][0:3]))

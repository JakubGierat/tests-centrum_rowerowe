...
class Cart:
    ...
    def product_price_value(self):
        Logger.info("Cart price: {}".format(float(''.join([i.replace('\xa0zł', '').replace(',', '.').strip(' ') for i in self.product_price.first.text_content().split('\n')]))))

        return float(''.join([i.replace('\xa0zł', '').replace(',', '.').strip(' ') for i in self.product_price.first.text_content().split('\n')]))

    def part_summary_value(self):
        Logger.info("Cart summary price: {}".format(float(''.join([i.replace(' zł', '').replace(',', '.').strip(' ') for i in self.part_summary.first.text_content().split('\n')]))))

        return float(''.join([i.replace(' zł', '').replace(',', '.').strip(' ') 
    ...
    def increase_number_of_product_pieces(self):
        if self.select_bar.is_visible():
            ...
            self.select_bar.select_option(label=self.select_bar_options.last.text_content().strip(' '))
            self.select_bar.click()
